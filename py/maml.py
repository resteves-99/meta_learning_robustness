...
class MAML:
    ...
    def _inner_loop(self, images, labels, train):   # pylint: disable=unused-argument
        ...
        if train:
            ...
            for i in range(self._num_inner_steps+1):
                ...
                grad = autograd.grad(outputs=loss, inputs=parameters.values(), create_graph=True)
                for idx in range(len(parameters)):
                    parameters[keys[idx]] = parameters[keys[idx]] - self._inner_lrs[keys[idx]] * grad[idx]
                curr_acc = util.score(preds, labels)
        ...
        # ******************* YOUR CODE HERE *********************
        # ********************************************************
        return parameters, accuracies, loss
    ...
    def _outer_step(self, task_batch, train):  # pylint: disable=unused-argument
        ...
        for task in task_batch:
            ...
            # conduct inner loop
            parameters, curr_support_accuracy, _ = self._inner_loop(images_support, labels_support, train=True)
            accuracies_support_batch.append(curr_support_accuracy)

            # conduct outer loop
            preds = self._forward(images_query, parameters)
            curr_outer_loss = F.cross_entropy(preds, labels_query)
            curr_query_accuracy = util.score(preds, labels_query)

            accuracy_query_batch.append(curr_query_accuracy)
            outer_loss_batch.append(curr_outer_loss)
