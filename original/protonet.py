...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            #shuffle query images and labels
            shuffled_idxs = torch.randperm(len(labels_query))
            labels_query = torch.clone(labels_query)
            images_query = torch.clone(images_query)
            labels_query = labels_query[shuffled_idxs]
            images_query = images_query[shuffled_idxs, :, :, :]

            # compute prototypes
            prototypes = [[] for i in range(len(torch.unique(labels_support)))]
            embed_support = self._network(images_support)
            for curr_embed, curr_label in zip(embed_support, labels_support):
                prototypes[curr_label].append(curr_embed)
            for curr_cluster in range(len(prototypes)):
                prototypes[curr_cluster] = torch.stack(prototypes[curr_cluster])
            prototypes = torch.stack(prototypes)
            prototypes = torch.mean(prototypes, 1)

            # assign query distances
            embed_query = self._network(images_query)
            all_dists = torch.cdist(embed_query, prototypes)**2

            # compute query metrics
            # compute loss
            curr_loss = F.cross_entropy(-all_dists, labels_query)
            loss_batch.append(curr_loss)
            # compute accuracies
            curr_batch_query_accuracy = util.score(-all_dists, labels_query)
            accuracy_query_batch.append(curr_batch_query_accuracy)

            # assign support distances
            all_dists = torch.cdist(embed_support, prototypes)**2

            # compute support accuracy
            curr_batch_support_accuracy = util.score(-all_dists, labels_support)
            # print(curr_batch_support_accuracy)
            accuracy_support_batch.append(curr_batch_support_accuracy)
            # ********************************************************
            # ******************* YOUR CODE HERE *********************
            # ********************************************************
        return (
